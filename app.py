...
st.markdown("""
    ...
    .link-container { display: flex; align-items: center; background-color: #2D3436; padding: 12px; border-radius: 10px; margin-top: 15px; text-decoration: none !important; transition: background 0.3s; }
    .link-container:hover { background-color: #3D4446; }
    </style>
...
def process_data(df):
    ...
    for _, row in df.iterrows():
        ...
        # 멤버 색상 판별
        m_color = "#3E4556" # 기본색
        if any(k in raw_text for k in ["노아", "NOAH", "💜"]): m_color = "#C294FB" # 보라
        elif any(k in raw_text for k in ["하민", "HAMIN", "🖤", "💚"]): m_color = "#B2EBC1" # 그린포인트
        elif any(k in raw_text for k in ["예준", "YEJUN", "💙"]): m_color = "#A2D2FF" # 스카이블루
        elif any(k in raw_text for k in ["밤비", "BAMBY", "🩷"]): m_color = "#FFB7D5" # 핑크
        elif any(k in raw_text for k in ["은호", "EUNHO", "❤️"]): m_color = "#FF8E8E" # 레드
        ...
        with st.form("vote_form", clear_on_submit=True):
            ...
            if st.form_submit_button("보드에 등록하기 💙"):
                if f_text:
                    ...
                    conn.update(worksheet="Sheet1", data=pd.concat([existing, new_row], ignore_index=True))
            ...
            def display_fn(data):
                if data.empty: st.info("소식이 없습니다. 💫")
                    ...
                    for idx, row in data.reset_index().iterrows():
                        with cols[idx % 2]:
                            # 멤버 색상 테두리 및 네온 효과
                            style = f"border: 1.5px solid {row['color']}; box-shadow: 0px 4px 15px {row['color']}33;"
                            ...
                            if row['link'] and not tweet_html:
                                st.markdown(f"<a href='{row['link']}' target='_blank' class='link-container' style='border-left: 4px solid {row['color']};'><span style='color:#A29BFE; font-weight:bold;'>🔗 참여 링크 이동</span></a>", unsafe_allow_html=True)
...
# --- [섹션 2: 팁 등록] ---
    ...
    with st.expander("➕ 새로운 팁 직접 등록하기"):
        with st.form("tip_form", clear_on_submit=True):
            t_title, t_app = st.columns(2)
            title = t_title.text_input("팁 제목")
            app = t_app.text_input("앱 이름")
            content = st.text_area("공략 내용")
            link = st.text_input("상세 링크")
            if st.form_submit_button("팁 등록하기"):
                if title and content:
                    existing = conn.read(worksheet="tips", usecols=list(range(4)))
                    new_tip = pd.DataFrame([{"title": title, "app_name": app, "content": content, "link": link}])
                    conn.update(worksheet="tips", data=pd.concat([existing, new_tip], ignore_index=True))
...
# --- [섹션 3: 커뮤니티 등록] ---
    ...
    with st.form("comm_form", clear_on_submit=True):
        ...
        if st.form_submit_button("메시지 남기기"):
            if nick and msg:
                existing = conn.read(worksheet="comments", usecols=list(range(3)))
                new_comm = pd.DataFrame([{"nickname": nick, "comment": msg, "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M')}])
                conn.update(worksheet="comments", data=pd.concat([existing, new_comm], ignore_index=True))
